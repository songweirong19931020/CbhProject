...
) AS sml order by wastedbytes desc limit 20) re;
    '''
...
for i in result:
    print(i[1])
    conn2 = psycopg2.connect(host="192.168.12.106", port=5432, user="postgres", password="postgres", database="postgres")
    ...
    cur2 = conn2.cursor()
    sql_gp1 = '''vacuum freeze {schem}.{table_name};'''.format(schem=i[0],table_name=i[1])
    cur2.execute(sql_gp1)
    conn2.commit()
    cur2.close()
    conn2.close()